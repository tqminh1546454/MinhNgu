  ...
  <div>
    ...
    <v-card class="pa-4 mb-4">
      <v-row dense>
        ...
        <v-col cols="12" sm="3"><v-select v-model="statusFilter" :items="[{title:'Tất cả',value:''},...ROOM_STATUS_OPTIONS]" label="Trạng thái" density="compact" @update:model-value="loadData" /></v-col>
      </v-row>
    </v-card>
    <v-card>
      <v-data-table-server :headers="headers" :items="items" :items-length="items.length" :loading="loading" items-per-page="-1">
        ...
        <template #item.actions="{ item }">
          ...
          <v-btn icon size="small" variant="text" @click="statusItem=item;showStatus=true"><v-icon>mdi-swap-horizontal</v-icon></v-btn>
        </template>
      </v-data-table-server>
    </v-card>
    ...
    <v-dialog v-model="showStatus" max-width="400">
      <v-card v-if="statusItem">
        <v-card-title class="pt-5 px-6">Đổi trạng thái — {{ statusItem.roomNumber }}</v-card-title>
        <v-card-text class="px-6">
          <v-select v-model="newStatus" :items="ROOM_STATUS_OPTIONS" label="Trạng thái mới" class="mb-2" />
          <v-text-field v-if="newStatus==='UNDER_MAINTENANCE'" v-model="maintenanceReason" label="Lý do bảo trì" />
        </v-card-text>
        <v-card-actions class="px-6 pb-5"><v-spacer /><v-btn variant="text" @click="showStatus=false">Hủy</v-btn><v-btn color="primary" variant="flat" @click="changeStatus">Cập nhật</v-btn></v-card-actions>
      </v-card>
    </v-dialog>
  </div>
</template>
...
import { ROOM_STATUS_OPTIONS } from '@/shared/utils/constants'
...
const headers = [
  ...
  { title: 'Trạng thái', key: 'status', width: 140 }, { title: '', key: 'actions', width: 100, sortable: false },
]
...
async function changeStatus() {
  try { await http.patch(`/api/rooms/${statusItem.value.id}/status`, { status: newStatus.value, maintenanceReason: maintenanceReason.value }); success('Đã cập nhật'); showStatus.value = false; loadData() } catch(e) { console.error(e) }
}

async function loadData() {
  loading.value = true
  try {
    const { data } = await http.get('/api/rooms', { params: { buildingId: buildingFilter.value, floor: floorFilter.value, status: statusFilter.value } })
    items.value = data
