...
async function submit() {
  ...
  loading.value = true
  try {
    await http.post('/api/maintenance-requests', { ...form.value, studentId: auth.user?.studentId, studentCode: auth.user?.studentCode, studentName: auth.user?.name, roomId: 'room-001', roomNumber: 'A101' })
